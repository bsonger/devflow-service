...
type WorkloadConfigStore interface {
	...
	List(ctx context.Context, filter WorkloadConfigListFilter) ([]domain.WorkloadConfig, error)
}
...
type workloadConfigPostgresStore struct{}

func NewWorkloadConfigPostgresStore() WorkloadConfigStore {
	return &workloadConfigPostgresStore{}
}

func (s *workloadConfigPostgresStore) Create(ctx context.Context, item *domain.WorkloadConfig) (uuid.UUID, error) {
...
func (s *workloadConfigPostgresStore) Get(ctx context.Context, id uuid.UUID) (*domain.WorkloadConfig, error) {
...
func (s *workloadConfigPostgresStore) Update(ctx context.Context, item *domain.WorkloadConfig) error {
	if err := validateWorkloadConfig(item); err != nil {
		return err
	}
...
func (s *workloadConfigPostgresStore) Delete(ctx context.Context, id uuid.UUID) error {
	now := time.Now()
	result, err := db.Postgres().ExecContext(ctx, `
...
func (s *workloadConfigPostgresStore) List(ctx context.Context, filter WorkloadConfigListFilter) ([]domain.WorkloadConfig, error) {
	query := `
		select id, application_id, environment_id, name, description, replicas, exposed, resources, probes, env, labels, workload_type, strategy, created_at, updated_at, deleted_at
