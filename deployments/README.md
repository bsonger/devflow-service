...
- release-rendered workloads inject `SERVICE_NAME` from the pod label `app.kubernetes.io/name` via the downward API, keep `OTEL_SERVICE_NAMESPACE` and `SERVICE_VERSION` as explicit environment variables, and expose `METRICS_PORT` when metrics are enabled; rollout automation should patch `SERVICE_VERSION` to the git SHA, image digest, or CI build identifier when it updates the image
