...
func buildManifestDoc(manifest manifestdomain.Manifest, applicationName string) ManifestDoc {
	return ManifestDoc{
		ID:                     manifest.ID,
		Name:                   buildManifestName(manifest, applicationName),
...
func buildManifestName(manifest manifestdomain.Manifest, applicationName string) string {
	prefix := strings.TrimSpace(applicationName)
	base := strings.TrimSpace(manifest.ImageTag)
	if base == "" {
		base = shortManifestImageRef(manifest.ImageRef)
	}
	if base == "" {
		base = manifest.ID.String()
	}
	if prefix == "" {
		return base
	}
...
func shortManifestImageRef(imageRef string) string {
