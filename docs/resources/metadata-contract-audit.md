...
| Tekton `PipelineRun` | `internal/manifest/service/manifest.go` (`buildManifestPipelineRun`, `submitManifestBuild`) | `devflow.manifest/id` (label and annotation) | `otel.devflow.io/trace-id`, `otel.devflow.io/parent-span-id` | manifest runtime reconciler, release-owned manifest writeback routes |
